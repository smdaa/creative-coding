...
class Particle
{
  public:
    ...
    Particle(const vec2 &position, const vec2 &velocity, float radius, Color color)
    ...
    void updateVelocity(vec2 force, float dt)
    {
        velocity += force * dt;
    }
...
class World
{
  public:
    ...
    World()
    {
        ...
        grid = std::vector<std::vector<float>>(gridNumRows, std::vector<float>(gridNumCols, 0.0f));
    ...
    void updateParticles(float dt)
    {
        ...
        {
            int rowIndex =
                static_cast<int>(std::min(std::max(particles[i].position.y / static_cast<float>(GRID_RESOLUTION), 0.0f),
                                          static_cast<float>(gridNumRows - 1)));
            int columnIndex =
                static_cast<int>(std::min(std::max(particles[i].position.x / static_cast<float>(GRID_RESOLUTION), 0.0f),
  ...
  public:
    ...
    void draw() override
    {
        ...
        if (DRAW_GRID)
        {
            for (int i = 0; i < world.gridNumRows; ++i)
            {
                for (int j = 0; j < world.gridNumCols; ++j)
                {
                    ...
                    vec2 start = vec2(x, y);
                    vec2 end = vec2(x + arrowLength * cos(angle), y + arrowLength * sin(angle));
        ...
        if (world.particles.size() > 2)
        {
            ...
            {
                mesh.appendColorRgba(PALETTE[colorIndex++ % (sizeof(PALETTE) / sizeof(PALETTE[0]))]);
            }
